	...
	var tests = map[string]struct {
		...
		"stop-error": {
			...
			mocks: func(ctrl *gomock.Controller) {
				...
				mockRuntime.EXPECT().Listen("tcp", ":49152").Return(httpListener, nil)
				mockRuntime.EXPECT().Listen("tcp", ":49153").Return(httpsListener, nil)
				mockRuntime.EXPECT().Listen("tcp", ":49154").Return(internalListener, nil)
